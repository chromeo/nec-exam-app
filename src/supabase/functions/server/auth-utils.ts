/**
 * Shared authentication utilities for server routes
 * 
 * This module provides centralized auth helpers to avoid duplication across route files.
 * 
 * ✅ SINGLE SOURCE OF TRUTH for:
 * - Supabase client instances (exported for use in routes)
 * - Authentication validation functions (requireAdmin, requireUser)
 * 
 * @module auth-utils
 */
...
/**
 * Validates admin access for a request
 * 
 * Checks admin status through multiple fallback methods:
 * 1. Existing user profile in database
 * 2. Supabase user metadata (user_metadata.is_admin or app_metadata.is_admin)
 * 3. Admin email whitelist
 * 
 * @param accessToken - JWT access token from Authorization header
 * @returns AuthResult with admin status, user data, or error
 * 
 * @example
 * ```typescript
