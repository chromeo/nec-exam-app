...
export interface ExamTemplate {
  ...
  title: string;
  description: string;
  edition: NecEdition; // NEW: NEC edition this template is based on (e.g., "NEC-2023")
  questionCategories: Record<string, number>; // Maps category name to question count
  ...
  displayOrder?: number; // For custom ordering in UI
  template_name?: string; // Optional field for legacy compatibility
  createdAt?: string;
  updatedAt?: string;
  createdBy?: string;
  updatedBy?: string;
  
  // Runtime-calculated fields (not stored)
