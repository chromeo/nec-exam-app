...
import type { ServerExamResult, CreditTransaction } from './types.ts';

// Import shared auth utilities
import { requireAdmin, requireUser, supabase } from './auth-utils.ts';

const userProfileApp = new Hono();

console.log('👤 routes-user-profile.tsx module loaded');
...
userProfileApp.get('/admin/users/:userId/exams', async (c) => {
  try {
    console.log('👤 routes-user-profile.tsx: GET /admin/users/:userId/exams');
...
userProfileApp.get('/admin/users/:userId/exam-history', async (c) => {
  try {
    console.log('👤 routes-user-profile.tsx: GET /admin/users/:userId/exam-history');
...
userProfileApp.post('/admin/users/:userId/credits', async (c) => {
  try {
    console.log('👤 routes-user-profile.tsx: POST /admin/users/:userId/credits');
...
userProfileApp.get('/user/profile/stats', async (c) => {
  try {
    console.log('👤 routes-user-profile.tsx: GET /user/profile/stats');
    ...
    const stats = await getUserProfileStats(authResult.userId!);
