...
import type { 
  ...
  KvItem, 
  AuthResult,
  NecEdition  // ✅ Added missing NecEdition type import
} from './types.ts';
...
exams.post('/admin/exam-templates/reorder', async (c) => {
  try {
    ...
    const { templateId, newOrder } = await c.req.json();
    
    if (!templateId || newOrder === undefined) {
      return c.json({
        success: false,
        error: 'Missing required fields'
      }, 400);
    ...
    console.error('Error reordering exam template:', error);
    return c.json({
      success: false,
      error: 'Failed to reorder exam template'
    }, 500);
  }
});
