...
guidelines.get('/guidelines', async (c) => {
  try {
    console.log('📖 routes-guidelines.ts: Fetching Guidelines.md from Supabase Storage...');
...
guidelines.post('/admin/guidelines/sync', async (c) => {
  try {
    console.log('📖 routes-guidelines.ts: POST /admin/guidelines/sync');
    ...
    // Read Guidelines.md from server filesystem
    try {
      const guidelinesPath = './Guidelines.md';
      console.log(`📖 Reading Guidelines.md from: ${guidelinesPath}`);
      
      const content = await Deno.readTextFile(guidelinesPath);
      
      if (!content || typeof content !== 'string') {
        console.error('❌ Invalid content read from file');
        return c.json({ success: false, error: 'Content is required and must be a string' }, 400);
      }

      console.log(`📖 Read ${content.length} bytes of Guidelines.md content from filesystem`);
      ...
      console.error('❌ Failed to read Guidelines.md from filesystem:', fileError);
      return c.json({
        success: false,
        error: `Failed to read Guidelines.md: ${fileError instanceof Error ? fileError.message : 'Unknown error'}`
      }, 500);
    }
  } catch (error) {
